...
def test_confirm_mapping_persists_immutably(client, approved_version_id):
    ...
    r = client.get(f"/api/v1/datasets/{dataset_id}/versions/{version_id}", headers=headers)
...
def test_confirm_mapping_rejects_unclassified(client, sample_csv):
    ...
    ds_resp = client.post("/api/v1/datasets", json={"name": unique_name, "description": "Test"}, headers=headers)
    if ds_resp.status_code != 201:
        pytest.fail(f"Failed to create dataset: {ds_resp.status_code} - {ds_resp.text}")
    dataset_id = ds_resp.json()["id"]

    with open(sample_csv, "rb") as f:
        r = client.post(f"/api/v1/datasets/{dataset_id}/upload", files={"file": f}, headers=headers)
    dataset_id = r.json()["dataset_id"]
    
    import time
    status = None
    for _ in range(30):
        status_resp = client.get(f"/api/v1/datasets/{dataset_id}/status", headers=headers)
    ...
    r = client.post(f"/api/v1/datasets/{dataset_id}/versions/{version_id}/confirm-mapping",
                        json={"mappings": bad_mapping}, headers=headers)
...
def test_remap_creates_new_version(client, approved_version_id):
    ...
    r = client.post(f"/api/v1/datasets/{dataset_id}/versions/{version_id}/remap/confirm",
                    json={"mappings": new_mapping}, headers=headers)
    assert r.status_code == 200
    new_version = r.json()
...
def test_remap_preview_returns_affected_charts(client, approved_version_id):
    ...
    r = client.post(f"/api/v1/datasets/{dataset_id}/versions/{version_id}/remap",
                    json={"mappings": changed}, headers=headers)
    assert r.status_code == 200
    body = r.json()
...
def test_original_version_semantic_map_unchanged_after_remap(client, approved_version_id):
    ...
    original = client.get(f"/api/v1/datasets/{dataset_id}/versions/{version_id}", headers=headers).json()
    ...
    refetched = client.get(f"/api/v1/datasets/{dataset_id}/versions/{version_id}", headers=headers).json()
