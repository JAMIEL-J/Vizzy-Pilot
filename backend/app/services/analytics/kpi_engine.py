...
def _generate_sales_kpis(df: pd.DataFrame, classification: ColumnClassification, semantic_map_json: Optional[str] = None, reader: Optional["DuckDBReader"] = None, total_rows: int = 0) -> List[KPI]:
    ...
    total_revenue = 0
    if revenue_col:
        total_revenue = _safe_sum(df, revenue_col, reader=reader)
        ...
        if df_ytd_curr is not None and not df_ytd_curr.empty:
            ytd_curr_rev = _safe_sum(df_ytd_curr, revenue_col, reader=reader)
            ytd_prev_rev = _safe_sum(df_ytd_prev, revenue_col, reader=reader) if df_ytd_prev is not None else 0
            trend = _calc_trend(ytd_curr_rev, ytd_prev_rev)
            if trend is not None:
                trend_label = "YoY (YTD)"
        
        if trend is None and has_trend and df_prev is not None:
            curr_rev = _safe_sum(df_curr, revenue_col, reader=reader)
            prev_rev = _safe_sum(df_prev, revenue_col, reader=reader)
        ...
        try:
            ...
            if df_full_prev_year is not None and not df_full_prev_year.empty:
                full_prev_rev = _safe_sum(df_full_prev_year, revenue_col, reader=reader)
                kpis.append(KPI(
    ...
    if quantity_col:
        total_quantity = _safe_sum(df, quantity_col, reader=reader)
        
        trend = None
        if has_trend and df_prev is not None:
            curr_qty = _safe_sum(df_curr, quantity_col, reader=reader)
            prev_qty = _safe_sum(df_prev, quantity_col, reader=reader)
    ...
    if revenue_col and total_orders > 0:
        ...
        if has_trend and df_prev is not None and df_curr is not None:
            ...
            curr_aov = _safe_sum(df_curr, revenue_col, reader=reader) / curr_orders if curr_orders > 0 else 0
            prev_aov = _safe_sum(df_prev, revenue_col, reader=reader) / prev_orders if prev_orders > 0 else 0
    ...
    if profit_col and revenue_col and total_revenue > 0:
        total_profit = _safe_sum(df, profit_col, reader=reader)
    ...
    if discount_col and revenue_col and total_revenue > 0:
        total_discount = _safe_sum(df, discount_col, reader=reader)
        # Check if discount is a ratio (mean < 1 means values like 0.2, 0.15)
        col_mean = _safe_mean(df, discount_col, reader=reader)
...
def _generate_churn_kpis(df: pd.DataFrame, classification: ColumnClassification, semantic_map_json: Optional[str] = None, reader: Optional["DuckDBReader"] = None, total_rows: int = 0) -> List[KPI]:
    ...
    if ticket_col:
        total_tickets = _safe_sum(df, ticket_col, reader=reader)
    ...
    if tech_ticket_col and tech_ticket_col in df.columns:
        total_tech_tickets = _safe_sum(df, tech_ticket_col, reader=reader)
        kpis.append(KPI(
    ...
    if admin_ticket_col and admin_ticket_col in df.columns:
        total_admin_tickets = _safe_sum(df, admin_ticket_col, reader=reader)
        kpis.append(KPI(
    ...
    if tech_ticket_col and admin_ticket_col and tech_ticket_col in df.columns and admin_ticket_col in df.columns:
        tech_total = _safe_sum(df, tech_ticket_col, reader=reader)
        admin_total = _safe_sum(df, admin_ticket_col, reader=reader)
...
def _generate_marketing_kpis(df: pd.DataFrame, classification: ColumnClassification, semantic_map_json: Optional[str] = None, reader: Optional["DuckDBReader"] = None, total_rows: int = 0) -> List[KPI]:
    ...
    # 1) Total Impressions
    total_imp = _safe_sum(df, imp_col, reader=reader) if imp_col else 0.0
    if imp_col:
        kpis.append(KPI(
            key="impressions",
    ...
    # 2) Total Clicks
    total_clicks = _safe_sum(df, click_col, reader=reader) if click_col else 0.0
    if click_col:
        kpis.append(KPI(
            key="clicks",
            title="Total Clicks",
    ...
    if conversion_rate is None and conv_col:
        ...
        if binary_share is not None:
            ...
            total_conv = _safe_sum(df, conv_col, reader=reader)
    ...
    for metric_col in classification.metrics:
        ...
        if role == 'percent_rate':
            ...
            kpi_value = _safe_mean(df, metric_col, reader=reader)
            kpi_format = 'number'
            title_prefix = 'Avg'
            ...
            kpi_value = _safe_sum(df, metric_col, reader=reader)
            kpi_format = 'currency'
            title_prefix = 'Total'
...
def _generate_finance_kpis(df: pd.DataFrame, classification: ColumnClassification, semantic_map_json: Optional[str] = None, reader: Optional["DuckDBReader"] = None, total_rows: int = 0) -> List[KPI]:
    ...
    if income_col:
        total_income = _safe_sum(df, income_col, reader=reader)
        kpis.append(KPI(
            key="total_income",
            title="Total Income",
            value=total_income,
    ...
    if expense_col:
        total_expense = _safe_sum(df, expense_col, reader=reader)
        kpis.append(KPI(
            key="total_expenses",
...
def _generate_hr_kpis(df: pd.DataFrame, classification: ColumnClassification, semantic_map_json: Optional[str] = None, reader: Optional["DuckDBReader"] = None, total_rows: int = 0) -> List[KPI]:
    ...
    if target_col and target_col in df.columns:
        positive_count = _count_target_positive(df, target_col, reader=reader)
    ...
    if salary_col and salary_col in df.columns:
        avg_salary = _safe_mean(df, salary_col, reader=reader)
        if avg_salary > 0:
            kpis.append(KPI(
                key="avg_income",
    ...
    if tenure_col and tenure_col in df.columns:
        avg_tenure = _safe_mean(df, tenure_col, reader=reader)
        kpis.append(KPI(
            key="avg_tenure",
    ...
    if performance_col and performance_col in df.columns:
        avg_perf = _safe_mean(df, performance_col, reader=reader)
    ...
    if satisfaction_col and satisfaction_col in df.columns:
        avg_sat = _safe_mean(df, satisfaction_col, reader=reader)
    ...
    if worklife_col and worklife_col in df.columns:
        avg_wl = _safe_mean(df, worklife_col, reader=reader)
    ...
    if training_col and training_col in df.columns:
        avg_training = _safe_mean(df, training_col, reader=reader)
        kpis.append(KPI(
            key="avg_training",
...
def _generate_logistics_kpis(df: pd.DataFrame, classification: ColumnClassification, semantic_map_json: Optional[str] = None, reader: Optional["DuckDBReader"] = None, total_rows: int = 0) -> List[KPI]:
    ...
    if delivery_time_col:
        avg_delivery = _safe_mean(df, delivery_time_col, reader=reader)
        kpis.append(KPI(
    ...
    if shipping_cost_col:
        total_cost = _safe_sum(df, shipping_cost_col, reader=reader)
        kpis.append(KPI(
    ...
    if late_col:
        positive_count = _count_target_positive(df, late_col, reader=reader)
        rate = (positive_count / len(df)) * 100 if len(df) > 0 else 0
    ...
    if inventory_col:
        inventory_total = _safe_sum(df, inventory_col, reader=reader)
        kpis.append(KPI(
...
def _generate_education_kpis(df: pd.DataFrame, classification: ColumnClassification, semantic_map_json: Optional[str] = None, reader: Optional["DuckDBReader"] = None, total_rows: int = 0) -> List[KPI]:
    ...
    if gpa_col:
        avg_gpa = _safe_mean(df, gpa_col, reader=reader)
        kpis.append(KPI(
            key="avg_gpa",
            title="Avg GPA",
    ...
    if attendance_col:
        ...
        if attendance_rate is None:
            attendance_rate = _safe_mean(df, attendance_col, reader=reader)
        kpis.append(KPI(
            key="attendance_rate",
...
def _generate_ecommerce_kpis(df: pd.DataFrame, classification: ColumnClassification, semantic_map_json: Optional[str] = None, reader: Optional["DuckDBReader"] = None, total_rows: int = 0) -> List[KPI]:
    ...
    if conversion_col:
        ...
        if conversion_rate is None:
            conversion_rate = _safe_mean(df, conversion_col, reader=reader)
        kpis.append(KPI(
            key="conversion_rate",
            title="Conversion Rate",
            value=round(float(conversion_rate), 2),
    ...
    if abandonment_col:
        ...
        if abandonment_rate is None:
            abandonment_rate = _safe_mean(df, abandonment_col, reader=reader)
        kpis.append(KPI(
...
def _generate_real_estate_kpis(df: pd.DataFrame, classification: ColumnClassification, semantic_map_json: Optional[str] = None, reader: Optional["DuckDBReader"] = None, total_rows: int = 0) -> List[KPI]:
    ...
    if price_col:
        avg_price = _safe_mean(df, price_col, reader=reader)
        kpis.append(KPI(
            key="avg_price",
            title="Avg Price",
    ...
    if dom_col:
        avg_dom = _safe_mean(df, dom_col, reader=reader)
        kpis.append(KPI(
...
def _generate_customer_support_kpis(df: pd.DataFrame, classification: ColumnClassification, semantic_map_json: Optional[str] = None, reader: Optional["DuckDBReader"] = None, total_rows: int = 0) -> List[KPI]:
    ...
    if resolution_col:
        avg_resolution = _safe_mean(df, resolution_col, reader=reader)
        kpis.append(KPI(
    ...
    if response_col:
        avg_response = _safe_mean(df, response_col, reader=reader)
        kpis.append(KPI(
    ...
    if csat_col:
        avg_csat = _safe_mean(df, csat_col, reader=reader)
        kpis.append(KPI(
            key="avg_csat",
            title="Avg CSAT",
    ...
    if sla_col:
        ...
        if sla_rate is None:
            sla_rate = _safe_mean(df, sla_col, reader=reader)
        kpis.append(KPI(
            key="sla_compliance",
...
def _generate_it_operations_kpis(df: pd.DataFrame, classification: ColumnClassification, semantic_map_json: Optional[str] = None, reader: Optional["DuckDBReader"] = None, total_rows: int = 0) -> List[KPI]:
    ...
    if uptime_col:
        ...
        if uptime_rate is None:
            uptime_rate = _safe_mean(df, uptime_col, reader=reader)
        kpis.append(KPI(
            key="uptime_rate",
            title="Uptime",
    ...
    if downtime_col:
        avg_downtime = _safe_mean(df, downtime_col, reader=reader)
        kpis.append(KPI(
            key="avg_downtime",
            title="Avg Downtime",
    ...
    if latency_col:
        avg_latency = _safe_mean(df, latency_col, reader=reader)
        kpis.append(KPI(
            key="avg_latency",
            title="Avg Latency",
    ...
    if cpu_col:
        avg_cpu = _safe_mean(df, cpu_col, reader=reader)
        kpis.append(KPI(
            key="avg_cpu",
...
def _generate_cybersecurity_kpis(df: pd.DataFrame, classification: ColumnClassification, semantic_map_json: Optional[str] = None, reader: Optional["DuckDBReader"] = None, total_rows: int = 0) -> List[KPI]:
    ...
    if vuln_col:
        total_vuln = _safe_sum(df, vuln_col, reader=reader)
        kpis.append(KPI(
    ...
    if risk_col:
        avg_risk = _safe_mean(df, risk_col, reader=reader)
        kpis.append(KPI(
            key="avg_risk_score",
    ...
    if mttr_col:
        avg_mttr = _safe_mean(df, mttr_col, reader=reader)
        kpis.append(KPI(
...
def _generate_generic_kpis(df: pd.DataFrame, classification: ColumnClassification, semantic_map_json: Optional[str] = None, reader: Optional["DuckDBReader"] = None, total_rows: int = 0) -> List[KPI]:
    ...
    if classification.metrics:
        ...
        total = _safe_sum(df, primary_metric, reader=reader)
        kpis.append(KPI(
            key="primary_total",
        ...
        avg = _safe_mean(df, primary_metric, reader=reader)
        kpis.append(KPI(
            key="primary_avg",
