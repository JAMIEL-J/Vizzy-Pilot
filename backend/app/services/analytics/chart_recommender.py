...
# ============================================================================
# DATA SANITIZATION — Central NaN/NaT/None cleaner for all chart output
# ============================================================================
...
def _sanitize_chart_data(data: Any) -> Any:
    ...
    Handles all chart data shapes:
      - [{name, value}]  (bar/pie/donut)
      - [{timestamp, date, value}]  (trend/line)
      - [{x, y}]  (scatter)
    ...
    for row in data:
        ...
        for k, v in row.items():
            if k in ('value', 'x', 'y', 'positive', 'negative', 'low', 'high'):
                ...
                # Pass through metrics dicts and other nested structures
                if isinstance(v, float) and (pd.isna(v) or v != v):
                    new_row[k] = 0.0
                else:
                    new_row[k] = v
...
def _metric_format_type(metric: Optional[str]) -> Optional[str]:
    if not metric:
        return None
    ...
    # These contain "rate" but are pay rates (currency), not statistical rates (percent).
...
def _safe_groupby_sum(df: pd.DataFrame, group_col: str, value_col: str, limit: int = 10) -> List[Dict]:
    ...
    try:
        # Drop rows where the group key or value is NaN
