...
- **Bug Fix (Field Deletion NameError)**: Added a local import of `DatasetVersion` in the `delete_canvas_field` endpoint inside `canvas_routes.py` to resolve a NameError crash (`"DatasetVersion" is not defined`) when deleting schema fields.
